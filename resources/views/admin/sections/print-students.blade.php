...
<head>
    ...
    <style>
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .info-box { background:#f9fafb; border:1px solid #e5e7eb; border-radius:6px; padding:8px 14px; }
...
<div class="no-print" style="text-align:right; margin-bottom:16px;">
    <button onclick="window.print()" style="background:#1e4d2b; color:#fff; border:none; padding:8px 18px; border-radius:6px; font-size:13px; cursor:pointer;">
        🖨️ Print
    </button>
    <button onclick="window.close()" style="background:#fff; color:#374151; border:1px solid #d1d5db; padding:8px 18px; border-radius:6px; font-size:13px; cursor:pointer; margin-left:8px;">
        Close
    </button>
</div>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                ...
                <div style="font-size:10px; color:#6b7280; font-family:serif;">{{ $enrollment->student->user->name_kh }}</div>
                @endif
            </td>
            <td style="font-family:monospace;">{{ $enrollment->student->student_id }}</td>
            <td>{{ $enrollment->student->program->code }}</td>
            <td>{{ $enrollment->student->batch ? 'Batch '.$enrollment->student->batch : '—' }}</td>
